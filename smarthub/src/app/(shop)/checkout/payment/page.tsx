...
export default function PaymentPage() {
  ...
  const pollStatus = useCallback(
    (gateway: "mtn-momo" | "airtel-money", refKey: string, orderId: string) => {
      ...
      pollRef.current = setInterval(async () => {
        ...
        if (attempts > maxAttempts) {
          stopPolling();
          setStatusMsg("Payment timed out. Please check your orders.");
          setPayStep("failed");
          return;
        }

        try {
          const res = await fetch(`/api/payments/${gateway}?${param}=${refKey}`);
          if (!res.ok) return;
    ...
    [stopPolling, clearCart, clearCheckout]
  );
  ...
  async function handleCompletePayment() {
    ...
    if ((paymentMethod === "momo" || paymentMethod === "airtel") && !phone.trim()) {
    ...
    try {
      ...
      if (!payment?.id) throw new Error("Payment record not created");

      if (paymentMethod === "momo" || paymentMethod === "airtel") {
        const gateway = paymentMethod === "momo" ? "mtn-momo" : "airtel-money";
        const gwRes = await fetch(`/api/payments/${gateway}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!gwRes.ok) {
          const err = await gwRes.json();
          throw new Error(err.error || "Gateway request failed");
        }
        ...
        clearCheckout();
        setPayStep("success");
        setStatusMsg("Order placed. Card payment will be processed separately.");
      }
    } catch (err) {
      setPayStep("failed");
  ...
  return (
    <>
      ...
      <main className="max-w-7xl mx-auto px-margin-mobile py-lg pb-28">
        ...
        {payStep === "waiting" && (
          <div className="max-w-lg mx-auto text-center py-xl space-y-lg">
            <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mx-auto" />
            <h2 className="font-headline-md text-headline-md text-on-surface dark:text-white">Processing Payment</h2>
            <p className="font-body-md text-on-surface-variant dark:text-outline">{statusMsg}</p>
            <p className="text-[12px] text-on-surface-variant dark:text-outline">You will be redirected automatically once payment is confirmed.</p>
            <button onClick={goToOrders} className="text-primary dark:text-inverse-primary font-label-md hover:underline">
        ...
        {payStep === "success" && (
          <div className="max-w-lg mx-auto text-center py-xl space-y-lg">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface dark:text-white">Payment Successful</h2>
            <p className="font-body-md text-on-surface-variant dark:text-outline">{statusMsg}</p>
            <button onClick={goToOrders} className="bg-primary text-on-primary h-12 px-8 rounded-full font-label-md text-label-md hover:brightness-110 active:scale-95 transition-all">
              View My Orders
            </button>
          </div>
        )}
        ...
        {payStep === "failed" && (
          <div className="max-w-lg mx-auto text-center py-xl space-y-lg">
            ...
            <h2 className="font-headline-md text-headline-md text-on-surface dark:text-white">Payment Failed</h2>
            <p className="font-body-md text-on-surface-variant dark:text-outline">{statusMsg}</p>
            <div className="flex gap-md justify-center">
              <button onClick={() => setPayStep("form")} className="bg-primary text-on-primary h-12 px-8 rounded-full font-label-md text-label-md hover:brightness-110 active:scale-95 transition-all">
                Try Again
              </button>
              <button onClick={goToOrders} className="border border-outline-variant text-on-surface h-12 px-8 rounded-full font-label-md text-label-md hover:bg-surface-variant/30 active:scale-95 transition-all">
                View Orders
              </button>
            </div>
          </div>
